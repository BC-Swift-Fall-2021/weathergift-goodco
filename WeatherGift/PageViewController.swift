...
//  WeatherGift
//
//  Created by Connor Goodman on 10/11/21.
//
...
class PageViewController: UIPageViewController {
    ...
    func loadLocations() {
        guard let locationsEncoded = UserDefaults.standard.value(forKey: "weatherLocations")
                as? Data else {
                    print("Warning: Could not load weatherLocations data from UserDefaults. This would always be the case the first time an app is installed, so if that's the case then ignore this error")
        ...
        if let weatherLocations = try? decoder.decode(Array.self, from: locationsEncoded) as [WeatherLocation] {
            ...
            print("ERROR: Couldn't decode datea read from UserDefaults")
        }
        if weatherLocations.isEmpty {
            //TODO: Get User Location For The First Element In weatherLocations
            weatherLocations.append(WeatherLocation(name: "", latitude: 20.21, longitude: 20.21))
        }
    }
